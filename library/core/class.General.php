...
class General
{
     ...
     * @param type $presentPage
     * @param type $presentLink
     * @return string
     */
    ...
    /**
     * 
     * @param type $accessPage
     * @param type $redirect
     * @return boolean
     */
    static function checkAccess($accessPage, $redirect = true)
    {
        if(isset($_SESSION['user_rights']) && $accessPage != '')
        {
            if(!array_key_exists($accessPage, $_SESSION['user_rights']))
            {
                if($redirect === true)
                {
                    ob_start();
                    header("Location: " . __SITE_URL . "admin/index");
                }
                else
                {
                    return false;
